...
### **问题 4：官方 browser 工具无法导航到域名**

**症状：** `openclaw browser navigate` 报错 "Navigation blocked: strict browser SSRF policy"

**原因：** OpenClaw 的 SSRF 策略要求使用 IP 地址，不支持域名导航

**解决方案 1：使用 CDP API 封装脚本（推荐）**
...
# 获取快照
./scripts/snapshot.sh t1

# 关闭标签页
./scripts/close-tab.sh <tab-id>
```

**解决方案 2：配置 SSRF 白名单（不推荐，降低安全性）**
...
| 脚本 | 用途 | 示例 |
|------|------|------|
| `open-url.sh` | 打开网页 | `./open-url.sh https://www.bilibili.com` |
| `list-tabs.sh` | 列出标签页 | `./list-tabs.sh` |
| `screenshot.sh` | 截图 | `./screenshot.sh` |
| `snapshot.sh` | 获取快照 | `./snapshot.sh t1` |
| `close-tab.sh` | 关闭标签页 | `./close-tab.sh <tab-id>` |
